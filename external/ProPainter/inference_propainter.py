...
def write_video_cv2_rgb(frames, file_path, fps):
    if len(frames) == 0:
        return
    ...
    if not writer.isOpened():
        raise RuntimeError(f"Failed to open video writer: {file_path}")
...
if __name__ == '__main__':
    ...
    masked_in_path = os.path.join(save_root, 'masked_in.mp4')
    inpaint_out_path = os.path.join(save_root, 'inpaint_out.mp4')
    try:
        # Explicit codec avoids PyAV default codec resolution failures.
        imageio.mimwrite(masked_in_path, masked_frame_for_save, fps=fps, codec='libx264')
        imageio.mimwrite(inpaint_out_path, comp_frames, fps=fps, codec='libx264')
    except Exception as exc:
        print(f'Warning: imageio video write failed ({exc}), falling back to OpenCV VideoWriter.')
        write_video_cv2_rgb(masked_frame_for_save, masked_in_path, fps)
        write_video_cv2_rgb(comp_frames, inpaint_out_path, fps)
