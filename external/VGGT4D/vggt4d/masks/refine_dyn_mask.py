...
class RefineDynMask:
    ...
    def _refine_mask(self, cam_id: int):
        ...
        dyn_pts_count = int(dyn_pts.shape[0])
        if dyn_pts_count == 0:
            # No dynamic candidates; fallback to coarse mask to keep pipeline running.
            return rearrange(coarse_mask > 0, "(h w) -> h w", h=h_img, w=w_img)

        n_clusters = min(30, dyn_pts_count)
        if n_clusters <= 1:
            dyn_pts_labels = np.zeros((dyn_pts_count,), dtype=np.int64)
        else:
            kmeans = KMeans(n_clusters=n_clusters, random_state=42)
            try:
                dyn_pts_labels = kmeans.fit_predict(dyn_pts.cpu().numpy())
            except ValueError:
                # Degenerate input for clustering; fallback to coarse mask.
                return rearrange(coarse_mask > 0, "(h w) -> h w", h=h_img, w=w_img)
