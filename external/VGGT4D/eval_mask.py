...
def load_gt_dyn_mask(gt_dyn_mask_paths: List[Path], merge_objects: bool = True) -> np.ndarray:
    """Load GT dynamic masks.
    
    Args:
        gt_dyn_mask_paths: List of paths to GT mask files
        merge_objects: If True, merge all object masks into a single binary mask.
            Any pixel belonging to any object is considered foreground.
    """
    dyn_masks = []
    for gt_dyn_mask_path in gt_dyn_mask_paths:
        dyn_mask = np.array(vggt_crop_img(Image.open(gt_dyn_mask_path)))
        if merge_objects:
            # Merge all objects: any pixel > 0 is foreground
            dyn_mask = (dyn_mask > 0).astype(np.uint8)
        dyn_masks.append(dyn_mask)
    return np.array(dyn_masks) > 0
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Evaluate mask predictions with optional GT object merging")
    parser.add_argument("--pred_root", type=str, default="outputs/ours", help="Prediction root directory")
    parser.add_argument("--gt_root", type=str, default="datasets/DAVIS/Annotations_unsupervised/480p", help="GT root directory")
    parser.add_argument("--merge_gt_objects", action="store_true", default=True, 
                        help="Merge all GT objects into a single binary mask (default: True)")
    parser.add_argument("--no_merge_gt_objects", action="store_false", dest="merge_gt_objects",
                        help="Do not merge GT objects (original behavior)")
    ...
    for seq_name in seq_names:
        ...
        gt_dyn_masks = load_gt_dyn_mask(gt_dyn_mask_paths, merge_objects=args.merge_gt_objects)
