...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--binary", action="store_true", help="Merge all masks into a single binary mask (foreground=1).")
...
def main() -> None:
    ...
    if binary_mode:
        out_frames = [(arr > 0).astype(np.uint8) for arr in frames]
        mode = "binary"
    else:
        all_labels = set()
        for arr in frames:
