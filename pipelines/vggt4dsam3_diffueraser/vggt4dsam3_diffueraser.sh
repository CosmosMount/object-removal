...
DIFFUERASER_DIR="${ROOT_DIR}/DiffuEraser"
...
PRED_FRAMES_DIR="${DIFFUERASER_OUT_ROOT}/frames"
...
conda run -n "${DIFFUERASER_ENV}" python "${ROOT_DIR}/pipeline_vggt4dsam3/postprocess_sam3.py" \
	...
	--inpaint_frames_dir "${DIFFUERASER_OUT_ROOT}/${VIDEO_NAME}/frames" \
...
DIFFUERASER_VIDEO_LENGTH="${DIFFUERASER_VIDEO_LENGTH:-${N_FRAMES}}"
...
if [[ "${MODE}" == "davis" && "${EVAL_DAVIS}" == "1" ]]; then
	...
	conda run -n "${DIFFUERASER_ENV}" python "${ROOT_DIR}/pipeline_vggt4dsam3/prepare_davis_eval_masks.py" \
...
METRICS_CMD=(
	conda run -n "${DIFFUERASER_ENV}" python "${ROOT_DIR}/evaluate_metrics.py"
